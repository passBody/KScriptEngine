...
（``.kscp`` 包，即 :class:`model.工程.kscp_package.KscpPackage`）中 ``assets/`` 目录下的文件。
...
from ..工程.path_util import norm_maybe_root
...
class _ImageType(VariableType):
    ...
    def normalize(self, data: Any) -> Any:
        """把路径规范成正斜杠、去前导 ``/``、折叠 ``.``、拒 ``..``（委托 :func:`model.工程.path_util.norm_maybe_root`）。"""
...
# ================================================================
# 冒烟演示：直接 ``python -m model.变量.project_variable`` 运行
# ================================================================
