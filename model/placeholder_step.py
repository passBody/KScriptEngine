...
本类继承 :class:`model.step.Step`，故可像普通步骤一样被卡片视图渲染、序列化、
...
# 合成卡片引用标记前缀（与 model.composite_card 一致；用于 _extract_name 识别）
_MARKER_PREFIX = "@合成卡片:"
...
class PlaceholderStep(Step):
    ...
    def run(self) -> int:
        ...
        手动置 ``ERROR``（同 :meth:`model.step.Step.do` 文档「run() 可手动置
...
# ================================================================
# 冒烟演示：直接 ``python -m model.placeholder_step`` 运行
# ================================================================
